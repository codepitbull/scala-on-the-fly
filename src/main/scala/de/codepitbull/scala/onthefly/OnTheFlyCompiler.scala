...
/**
  * This compiler is used to compile bith scriplets and scala-file contained in the classpath.
    ...
    synchronized {
      classCache.get(className).orElse {
        classLoader.tryToLoadClass(className) match {
          case Some(c) => Some(c)
          case None => {
            val res = classLoader.getResourceAsStream(className.replace(".","/") + ".scala")
            if(res == null)
              None
            else {
